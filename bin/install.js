...
if (!hasNext) {
  ...
  console.error(
    "  Bootstrap a Next.js project first: npx create-next-app@latest",
  );
  process.exit(1);
}
...
if (existsSync(srcAppDir) && !existsSync(featuresDir)) {
  ...
  // Copy features
  cpSync(join(templatesSrc, "features", "landing"), featuresDir, {
    recursive: true,
  });
  ...
  cpSync(
    join(templatesSrc, "app", "api", "lead", "route.ts"),
